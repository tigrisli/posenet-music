...
def display_in_cv_image(image,poses):
  ...
  #thresholds
  left_knee_threshold = 50
  right_knee_threshold = 50
  left_wrist_threshold = 50
  right_wrist_threshold = 50
  left_elbow_threshold = 50
  right_elbow_threshold = 50
  ...
  for pose in poses:
    ...
    for label, keypoint in pose.keypoints.items():
      if label.name == "LEFT_KNEE" and keypoint.score > 0.1:
        ...
        if left_knee_current_x > left_knee_previous_x+left_knee_threshold:
          play_drum_beats(r)
      ...
      if label.name == "RIGHT_KNEE" and keypoint.score > 0.1:     
        ...
        if right_knee_current_x > right_knee_previous_x+right_knee_threshold:
          play_drum_beats(r)
      ...
      if label.name == "LEFT_WRIST" and keypoint.score > 0.1:          
        ...
        if left_wrist_current_x > left_wrist_previous_x+left_wrist_threshold:
      ...
      if label.name == "RIGHT_WRIST" and keypoint.score > 0.1:          
        ...
        if right_wrist_current_x > right_wrist_previous_x+right_wrist_threshold:
          right_wrist_previous_x = right_wrist_current_x  
          play_random_guitar_note_threaded()   
          last_time=current_time          
          
        if right_wrist_current_x < right_wrist_previous_x-right_elbow_threshold:
          right_wrist_previous_x = right_wrist_current_x 
          play_random_guitar_note_threaded() 
          last_time=current_time            
      ...
      if label.name == "RIGHT_ELBOW" and keypoint.score > 0.1:          
        ...
        if right_elbow_current_x > right_elbow_previous_x+right_elbow_threshold:
      ...
      if label.name == "LEFT_ELBOW" and keypoint.score > 0.1:          
        ...
        if left_elbow_current_x > left_elbow_previous_x+left_elbow_threshold:
